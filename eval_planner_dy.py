...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Shrink CEM horizon near the fixed dataset goal: "
            "effective_horizon = min(horizon, max(1, goal_offset_steps - current_step))."
        ),
    )
    parser.add_argument(
        "--no-dynamic-horizon",
        dest="dynamic_horizon",
        action="store_false",
        help="Use the fixed --horizon value for every planning step.",
    )
    parser.set_defaults(dynamic_horizon=True)
    ...
    base_horizon = int(args.horizon)

    for step_idx in range(args.max_steps):
        if args.dynamic_horizon:
            remaining_to_goal = int(args.goal_offset_steps) - int(step_idx)
            planner.cfg.horizon = min(base_horizon, max(1, remaining_to_goal))
