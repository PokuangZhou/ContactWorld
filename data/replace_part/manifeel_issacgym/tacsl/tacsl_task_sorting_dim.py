...
class TacSLTaskSortingDim(TacSLTaskImageAugmentation, TacSLEnvSorting, FactoryABCTask):

    DEFAULT_CAMERA_OBS_DARKEN_FACTOR = 0.4
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.camera_obs_darken_factor = self.cfg_task.env.get(
            "camera_obs_darken_factor",
            self.DEFAULT_CAMERA_OBS_DARKEN_FACTOR,
        )
        self.camera_obs_darken_keys = [
            c_cfg.name
            for c_cfg in self.cfg_task.env.get("camera_configs", [])
            if c_cfg.get("image_type", "rgb") == "rgb"
        ]
    ...
    def _darken_camera_obs(self):
        """Dim regular RGB camera observations."""
        if self.camera_obs_darken_factor == 1.0:
            return

        for cam in self.camera_obs_darken_keys:
            if cam in self.obs_dict:
                self.obs_dict[cam][..., :3].mul_(self.camera_obs_darken_factor).clamp_(0.0, 1.0)
