#!/bin/bash
#SBATCH --job-name=eval_wm-usb-l4c
...
EVAL_VARIANT=${EVAL_VARIANT:-front}

case "${EVAL_VARIANT}" in
    front)
        DEFAULT_CONFIG=${CONTACTWORLD_ROOT}/config/tf/eval/dino/eval_planner_env_dino_front_last4conc.yaml
        ;;
    front_rgb|rgb)
        DEFAULT_CONFIG=${CONTACTWORLD_ROOT}/config/tf/eval/dino/eval_planner_env_dino_front_rgb_last4conc.yaml
        ;;
    *)
        echo "Unknown EVAL_VARIANT=${EVAL_VARIANT}; use front, front_rgb, or set CONFIG_FILE directly." >&2
        exit 2
        ;;
esac
...
echo "Eval last4conc job started on $(hostname) at $(date)"
...
echo "Eval last4conc job finished at $(date)"
