...
def sample_init_goal_segments(
    ...
    result = {
        ...
        "init_case_id": dataset.get_col_data("case_id")[init_rows],
        "goal_case_id": dataset.get_col_data("case_id")[goal_rows],
    }
...
def reset_env_to_dataset_state(env, batch: dict):
    kwargs = dict(
...
def get_gt_modality_sequence(dataset: ZarrDataset, batch: dict, key: str, env_i: int, num_frames: int):
    ...
    s = int(batch["start_step"][env_i])

    start_row = int(dataset.offsets[ep] + s)
    rows = np.arange(start_row, start_row + num_frames)

    data = dataset.get_col_data(key)
...
def save_matched_rollout_and_gt_videos(
    ...
    for key, frames in rollout_frames.items():
        ...
        for env_i in range(record_envs):
            ...
            # 如果 stop-on-success 提前结束，rollout 会短于 GT；
            # 这里用最后一帧补齐，保证 rollout 和 GT 视频时间长度一致。
            if rollout_seq.shape[0] < target_num_frames:
                pad_num = target_num_frames - rollout_seq.shape[0]
                last_frame = rollout_seq[-1:]
                pad_frames = np.repeat(last_frame, pad_num, axis=0)
                rollout_seq = np.concatenate([rollout_seq, pad_frames], axis=0)
            ...
            rollout_vis_frames = render_modality_frames(
                seq=rollout_seq,
                vis_type=vis_type,
            )

            gt_vis_frames = render_modality_frames(
                seq=gt_seq,
                vis_type=vis_type,
            )

            combined_frames = _concat_rollout_gt_frames(
                rollout_vis_frames,
                gt_vis_frames,
            )
            ...
            print(f"Saved rollout-vs-GT video to: {output_path}")
...
def main() -> None:
    ...
    # Action sampling prior for CEM. This does not use future GT action chunks.
    parser.add_argument(
        "--use-action-prior",
        action="store_true",
    )
    ...
    parser.add_argument("--quat-thresh-deg", type=float, default=180.0)
